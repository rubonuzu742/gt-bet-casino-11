...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-277d42e4";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-277d42e4">'+'<span class="tile-face-277d42e4 tile-front-277d42e4"></span>'+'<span class="tile-face-277d42e4 tile-back-277d42e4"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-277d42e4")){return;}
var back=tile.querySelector(".tile-back-277d42e4");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-277d42e4");if(isMine){tile.classList.add("boom-277d42e4");}
if(dim){tile.classList.add("dimmed-277d42e4");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-277d42e4")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-277d42e4");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-277d42e4")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-277d42e4");void panelEl.offsetWidth;panelEl.classList.add("jolt-277d42e4");}
endRound();}
